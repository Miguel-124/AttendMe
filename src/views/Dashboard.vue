<template>
      ...
      <div class="container">
        ...
        <ul class="session-list">
          ...
          >
            ...
            <!-- Przycisk szczegółów -->
            <button class="btn-details" @click="openSession(session.courseSessionId)">
              Szczegóły
            </button>
          </li>
        </ul>
  ...
  // Pobranie danych użytkownika – ten sam mechanizm dla obu ról
  ...
  // Pobranie sesji – w zależności od roli wywołujemy odpowiedni endpoint
  async function fetchSessions() {
    ...
    try {
      // Wybieramy endpoint w zależności od roli
      const url =
        userRole.value === "Nauczyciel"
  ...
  const filteredSessions = computed(() => {
    return sessions.value.filter((session) => {
      const matchesSearch =
        ...
        session.locationName.toLowerCase().includes(searchText.value.toLowerCase()) ||
        session.courseGroupName.toLowerCase().includes(searchText.value.toLowerCase());
  ...
  <style scoped>
  /* Przykładowe style – możesz dostosować według potrzeb */
